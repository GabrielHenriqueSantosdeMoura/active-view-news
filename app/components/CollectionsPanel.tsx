...
import { CollectionData, SavedArticleData } from '../lib/database.types';
...
interface CollectionsPanelProps {
  ...
  onRemoveFromCollection: (collectionName: string, articleUrl: string) => void;
  isLoading?: boolean;
}
...
export default function CollectionsPanel({
  ...
  const selectedCollectionData = collections.find(c => c.collectionName === selectedCollection);
  const selectedArticles = selectedCollectionData?.articles || [];

  const totalArticles = collections.reduce((sum, c) => sum + c.articles.length, 0);
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {isLoading ? (
            <div className="flex items-center justify-center py-12">
              <LoaderIcon size={32} className="text-[var(--primary)]" />
            </div>
          ) : selectedCollection ? (
            ...
            <div className="space-y-4">
              ...
              {selectedArticles.length === 0 ? (
                ...
                <div className="space-y-3">
                  {selectedArticles.map((article: SavedArticleData) => (
                    <div
                      key={article.url}
                    ...
                    >
                      <div className="flex gap-3">
                        ...
                        <div className="flex-shrink-0 flex flex-col gap-2">
                          ...
                          <button
                            onClick={() =>
                              onRemoveFromCollection(selectedCollection, article.url)
                            }
            ...
            <div className="space-y-3">
              {collections.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-[var(--text-muted)] mt-1">
                    Save articles to create your first collection
                  </p>
                </div>
              ) : (
          ...
          <>
            ...
            <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 bg-white rounded-2xl shadow-2xl z-50 p-6 animate-fade-in">
              ...
              <p className="text-[var(--text-muted)] mb-6">
                This action cannot be undone. All saved articles in &ldquo;{deleteConfirm}&rdquo; will be removed.
              </p>
              <div className="flex gap-3">
                <button
